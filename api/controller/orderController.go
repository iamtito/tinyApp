...
func AllOrders(c *fiber.Ctx) error {
	...
	log.Println(c, page)
	return c.JSON(users.Paginate(database.DB, &users.Order{}, page))
}
...
func Chart(c *fiber.Ctx) error {
	var sales []Sales
	database.DB.Raw(`SELECT DATE_FORMAT(o.created_at,'%Y-%m-%d') as date , SUM(oi.price * oi.quantity) as sum
	FROM orders o
